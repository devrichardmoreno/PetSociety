...
export class DiagnosesHistoryModal implements OnInit{
  ...
  get petInfo(): DiagnoseDto | null {
    return this.lastestDiagnoses.length > 0 ? this.lastestDiagnoses[0] : null;
  }
  ...
  loadDiagnoses(page: number = 0): void {
    ...
    .subscribe({
      ...
      error: (err) => {
        // 404 no es un error, es simplemente que no hay diagnósticos
        if(err.status === 404){
          this.lastestDiagnoses = [];
          this.diagnosesPage.totalPages = 0;
          this.diagnosesPage.totalElements = 0;
          this.diagnosesPage.page = 0;
          this.diagnosesError = null; // No es un error, es un estado vacío
        }
